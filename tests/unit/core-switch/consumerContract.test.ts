...
function validCapabilities(): Record<string, unknown> {
  return {
    contract: { rest: '1', websocket: '1', startup: '2' },
    feature_version: '1',
    features: {
...
describe('runCentaurConsumerContract', () => {
  ...
  it('rejects a Core without the versioned dual-startup capability', async () => {
    ...
    capabilities.contract = { rest: '1', websocket: '1', startup: '1' };
...
describe('verifyCoreMigrationCount', () => {
  ...
  it('accepts exactly 23 successful startup migrations and rejects an incomplete set', async () => {
    ...
    incomplete.exec('DELETE FROM _sqlx_migrations WHERE version = 23');
    incomplete.close();
    await expect(verifyCoreMigrationCount(dataDir)).rejects.toThrow('expected 23 successful Core migrations');
  });
});
