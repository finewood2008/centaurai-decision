...
const PATH_PATTERN = new RegExp(
  [
    String.raw`file:\/\/[^\s<>"'\x60]+?\.(?:${EXT})\b`,
    String.raw`["'\x60]([^"'\x60]+?\.(?:${EXT}))["'\x60]`,
    String.raw`(?<![A-Za-z0-9:/])(?:~|\/|\.{1,2}[\\/]|[A-Za-z]:[\\/])[^<>"'\x60\r\n]*?\.(?:${EXT})\b`,
    String.raw`(?:^|[\s:：,，(（\[])([^\s<>"'\x60\/\\:：,，;；)）\]]+?\.(?:${EXT}))(?=$|[\s,，.。;；:)）\]])`,
  ].join('|'),
  'gi'
);
...
function clean(raw: string): string {
  ...
  try {
    ...
    // Keep malformed URI text unchanged; later validation will reject it.
  }
  return value;
}
...
export function extractGeneratedArtifactPaths(value: unknown, depth = 0): string[] {
  ...
  if (typeof value === 'string') {
    return dedupe([...value.matchAll(PATH_PATTERN)].map((match) => match.slice(1).find(Boolean) || match[0]));
  }
  ...
  return dedupe(
    Object.entries(value).flatMap(([key, nested]) => {
      ...
      if (typeof nested === 'string' && EXPLICIT_PATH_KEY.test(key)) {
        if (!/\s/.test(nested.trim()) && EXT_PATTERN.test(clean(nested))) return dedupe([nested]);
