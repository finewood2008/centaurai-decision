...
export function hasUsableBillingUpstream(
  upstream?: Partial<BillingUpstreamAuth> | null
): upstream is BillingUpstreamAuth {
...
export function useBillingProviderKeys(params?: BillingProviderKeysRequest) {
  return useSWR(
    hasUsableBillingUpstream(params?.upstream) ? [BILLING_PROVIDER_KEYS_KEY, params] : null,
    () => ipcBridge.billing.providerKeys.invoke(params!),
    {
...
export function useBillingSummary(query: BillingQuery) {
  return useSWR(
    hasUsableBillingUpstream(query.upstream) ? [BILLING_SUMMARY_KEY, query] : null,
    () => ipcBridge.billing.summary.invoke(query),
    {
...
export function useBillingEvents(query: BillingQuery) {
  return useSWR(
    hasUsableBillingUpstream(query.upstream) ? [BILLING_EVENTS_KEY, query] : null,
    () => ipcBridge.billing.events.invoke(query),
    {
      refreshInterval: 10_000,
    }
  );
...
export function useBillingTimeseries(query: BillingQuery) {
  return useSWR(
    hasUsableBillingUpstream(query.upstream) ? [BILLING_TIMESERIES_KEY, query] : null,
    () => ipcBridge.billing.timeseries.invoke(query),
    {
      refreshInterval: 10_000,
    }
  );
...
export function providerToBillingUpstream(provider?: {
  base_url?: string;
  api_key?: string;
}): BillingUpstreamAuth | undefined {
