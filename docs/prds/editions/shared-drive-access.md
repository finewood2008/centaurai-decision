# 超级参谋团访问团队版共享盘（F-SHARE）

> **状态：📋 计划中，目标排期下个版本。本文档为设计与实施计划，尚未实现。**
>
> 本文档定义**数据平面 v1**：把团队版（团队服务器）当作共享盘服务器，让超级参谋团（另一台机器）
> 通过局域网访问**同一个共享盘**，实现跨机器数据共享。
>
> **本期范围**：仅「两台机器看到同一个共享盘」。**汇报 / intel 收件箱 / 联络 agent 等功能本期不做**
> （产品方决定 2026-06：「目前先不做汇报功能，先让团队版跟超级参谋团都能看到同个共享盘」）。
...
- **作为老板（超级参谋团）**，我希望能直接浏览/下载团队那台机器上的共享盘内容，和团队看到的是同一份数据。
- **作为团队（团队版）**，我这台机器本来就是共享盘服务器（员工通过浏览器访问），现在让老板的机器也能接进来看。

### 1.2 目标（本期）

1. 团队版作为共享盘服务器（**已具备**，见 §3）。
2. 超级参谋团作为共享盘**客户端**，经局域网访问团队版的同一个共享盘（浏览 / 下载，视需要上传）。
...
| N2  | 超级参谋团访问团队盘的鉴权          | **专用服务账号 token（可吊销）**           | 独立于员工账号，可单独吊销；复用现有 `webui_gate` token 机制 |
...
```
      决策机 (超级参谋团 · 回环)                  团队服务器 (团队版 · 0.0.0.0:25808)
  ┌──────────────────────────┐          ┌─────────────────────────────┐
...
团队的 `/api/shared-drive/*`（和可选 `/api/nas/*`）→ 在超级参谋团 `/files` 里浏览/下载团队的同一份共享盘。
...
也可在 OS 层把团队磁盘以 SMB/NFS 挂载，再把超级参谋团 `nasRootDir` 指向挂载点——**几乎零应用代码**，但需各平台 OS 级网络共享配置、绕开应用鉴权门、IT 负担重、体验割裂。**本期采用应用层 HTTP 客户端方案（§6.3）**，与现有鉴权门和发现机制一致。

### 6.6 edition 门控（D6 沿用）

- 代码放核心仓库，`IS_DECISION`/`IS_TEAM` 门控：
  - 超级参谋团编译：远程共享盘客户端 + 连接配置 UI。
...
- [ ] 超级参谋团 `/files` 出现「团队共享盘」源，可浏览团队共享库的分类与文件。
- [ ] 超级参谋团可下载 / 预览团队共享盘文件（两端看到同一份数据）。
